...
void* better_general_par_merge_sort(void* data) {
    ...
    for(int i = 0; i < split-1; i++) pthread_create(pids + i, NULL, general_par_merge_sort, proxy_general_args(arrays[i], sizes[i], split, limiter-1));
    for(int i = 0; i < split; i++) merge_indecies[i] = 0;
    general_par_merge_sort(proxy_general_args(arrays[split-1], sizes[split-1], split, limiter-1));
    for(int i = 0; i < split-1; i++) pthread_join(pids[i], NULL);
